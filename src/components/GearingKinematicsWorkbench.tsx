...
export const GearingKinematicsWorkbench: React.FC<GearingKinematicsWorkbenchProps> = ({
  ...
  const ringRpm = Math.round(countershaftRpm * (46 / 74));  // 74T ring -> 46T countershaft ratio: 46/74
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow-lg space-y-4">
        ...
        <div className="p-4 rounded-lg bg-slate-950 border border-slate-800 grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-2.5 text-center">
          ...
          <div className="p-2 rounded bg-slate-900/80 border border-slate-800/80">
            ...
            <span className="text-[9px] text-emerald-500 block font-mono">27T (ρ=2.556)</span>
          </div>
          <div className="p-2 rounded bg-slate-900/80 border border-slate-800/80">
            ...
            <span className="text-[9px] text-green-500 block font-mono">74T (内69T)</span>
          </div>
          <div className="p-2 rounded bg-slate-900/80 border border-slate-800/80">
            ...
            <span className="text-[9px] text-cyan-500 block font-mono">46T/70T/12T</span>
          </div>
          <div className="p-2 rounded bg-slate-900/80 border border-slate-800/80">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 bg-slate-900 border border-slate-800 rounded-xl p-5 shadow space-y-4">
          <div className="flex items-center justify-between border-b border-slate-800 pb-3">
            <div>
              ...
              <p className="text-[11px] text-slate-400">
                满足转速共线方程：(1 + ρ)·n_c = n_s + ρ·n_r，杠杆比 SC : CR = 2.556 : 1
              </p>
            </div>
            <span className="text-xs font-mono text-slate-400">ρ = 69/27 = 2.556</span>
          </div>
          ...
          <div className="w-full bg-slate-950 p-4 rounded-lg border border-slate-800/80">
            <svg viewBox="0 0 540 240" className="w-full h-auto select-none">
              ...
              {/* C: Carrier (ICE) X=356 (Ratio SC : CR = 2.556 : 1) */}
              <line x1="356" y1="25" x2="356" y2="215" stroke="#475569" strokeWidth="1.5" />
              <text x="356" y="232" fill="#38bdf8" fontSize="11" fontWeight="bold" textAnchor="middle">
                C (行星架/ICE)
              </text>

              {/* R: Ring Gear (Output) X=456 */}
              <line x1="456" y1="25" x2="456" y2="215" stroke="#475569" strokeWidth="1.5" />
              <text x="456" y="232" fill="#22c55e" fontSize="11" fontWeight="bold" textAnchor="middle">
                R (齿圈输出)
              </text>

              {/* Distance dimension indicators */}
              <text x="228" y="16" fill="#94a3b8" fontSize="10" textAnchor="middle" fontFamily="monospace">
                距离比 = ρ (2.556)
              </text>
              ...
              {(() => {
                ...
                return (
                  <g>
                    ...
                    <line x1="100" y1={yS} x2="456" y2={yR} stroke="#facc15" strokeWidth="3" />
                    ...
                    {/* Point C (Carrier) */}
                    <circle cx="356" cy={yC} r="6" fill="#0284c7" stroke="#ffffff" strokeWidth="2" />
                    <rect x="326" y={yC - 24} width="60" height="18" rx="3" fill="#1e293b" />
                    <text x="356" y={yC - 12} fill="#bae6fd" fontSize="10" fontWeight="bold" textAnchor="middle" fontFamily="monospace">
                      {carrierRpm}
                    </text>

                    {/* Point R (Ring) */}
                    <circle cx="456" cy={yR} r="6" fill="#16a34a" stroke="#ffffff" strokeWidth="2" />
                    <rect x="426" y={yR - 24} width="60" height="18" rx="3" fill="#1e293b" />
                    <text x="456" y={yR - 12} fill="#86efac" fontSize="10" fontWeight="bold" textAnchor="middle" fontFamily="monospace">
                      {ringRpm}
                    </text>
                  </g>
                );
          ...
          <div className="text-[11px] text-slate-400 bg-slate-950/60 p-3 rounded-lg border border-slate-800 space-y-1">
            ...
            <div>• <b>高效巡航 (100 km/h)</b>：车速固定使 R 点稳定，发动机控制在 3400 rpm 最优热效率岛（C点），MG1（S点）处于 2656 rpm 高效发电工况，回充电池并驱动 MG2。</div>
          </div>
        </div>
        ...
        <div className="lg:col-span-5 bg-slate-900 border border-slate-800 rounded-xl p-5 shadow space-y-4">
          ...
          <div className="p-3.5 rounded-lg bg-slate-950 border border-slate-800 space-y-1.5">
            ...
            <div className="text-[11px] text-slate-300 font-mono space-y-0.5">
              <div>• 齿圈外齿分度圆: d_ro = 74 × 1.75 = 129.50 mm</div>
              <div>• 副轴受动齿分度圆: d_c1 = 46 × 1.75 = 80.50 mm</div>
              <div className="text-emerald-400 font-bold pt-1 border-t border-slate-800 flex justify-between">
                <span>中心距 a₁ = (129.50 + 80.50) / 2 = 105.00 mm</span>
                <span>速比 i₁ = 46/74</span>
              </div>
            </div>
          </div>
